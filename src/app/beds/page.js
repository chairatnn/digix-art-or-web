...
export default function BedsPage() {
  ...
  const loadBeds = async (query = "", showLoading = true) => {
    ...
    try {
      ...
      const url = `${API_BASE}/api/beds?search=${encodeURIComponent(query)}`;
  ...
  const handleDelete = async (id, bedNumber) => {
    if (!confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบเตียงหมายเลข "${bedNumber}"?`))
      return;
    try {
      const token = localStorage.getItem("accessToken");
      ...
      // });
      const resp = await fetch(`${API_BASE}/api/beds/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (resp.ok) {
      ...
      <main className="space-y-6 p-4 md:p-6">
        <section className="overflow-hidden rounded-[30px] bg-slate-200 px-6 py-7 shadow-2xl shadow-blue-200/60 md:px-8 md:py-9">
          ...
          <p className="mt-2 text-sm font-medium">
            แสดงสถานะและจัดการเตียงตามการจองคิวห้องผ่าตัด
          </p>
        </section>
        ...
        <div className="flex flex-col gap-4 border-b border-blue-600 pb-4 md:flex-row md:items-center md:justify-between">
          ...
          {canManage && (
            <Link
              href="/beds/add"
              className="inline-flex items-center gap-2 rounded-2xl bg-amber-400 px-4 py-2.5 text-s font-bold text-white shadow-lg shadow-blue-200 transition hover:-translate-y-0.5"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {beds.length === 0 ? (
            ...
            beds.map((bed) => (
              <div
                key={bed.id}
                className="flex flex-col gap-3 rounded-[26px] border border-slate-200/70 bg-white/90 p-5 shadow-lg shadow-blue-100/20 transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-blue-100/40"
              >
                <div className="flex justify-between items-start">
                  <h3 className="text-l font-extrabold text-slate-800">
                    {bed.bed_number}
                  </h3>
                  <div
                    className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${getStatusClasses(getDisplayStatus(bed.status))}`}
                  >
                ...
                {canManage && (
                  <div className="flex gap-2 mt-auto pt-4 border-t">
                    <Link
                      href={`/beds/${bed.id}`}
                      className="flex-1 text-center rounded-2xl bg-slate-100 py-2 text-sm font-semibold !text-blue-700 transition hover:bg-slate-200"
                    >
                      <Pencil className="h-4 w-4 inline" /> แก้ไข
                    </Link>
                    <button
                      onClick={() => handleDelete(bed.id, bed.bed_number)}
                      className="rounded-2xl bg-rose-50 px-3 py-2 text-sm text-rose-600 transition hover:bg-rose-100"
                    >
